...
- **Atomicity**: multi-row cascade deletes (deleting a watchlist and its items, or a profile and everything it owns) use `session.flush()` to send child-row deletes to Postgres in the correct order, but hold everything in one transaction with a single `commit()` at the end — so the whole cascade succeeds or rolls back together. An earlier version used separate `commit()` calls per tier, which got the ordering right but wasn't actually atomic: a crash mid-cascade could leave a partially-deleted state. Fixed and re-verified with real foreign-key enforcement.
- **Consistency**: enforced at two levels — a DB-level unique constraint on `(watchlist_id, symbol)` for fresh deployments, plus an explicit API-level check (case-insensitive) that protects an already-running database `create_all()` can't retroactively alter. Foreign keys between `watchlist` → `watchlistitem` are real Postgres constraints, not just convention.
- **Isolation**: default Postgres `READ COMMITTED`. This is a deliberate fit for the actual access pattern here (per-user, per-watchlist operations with no cross-user contention on the same rows), not an oversight — a stricter isolation level would add overhead without protecting against a race condition that can actually occur in this schema.
- **Durability**: Postgres is the only durable store — Redis is explicitly disposable cache with short TTLs, never the source of truth for anything. If Redis is wiped entirely, the app degrades to "no cached price until the next poll," not data loss, because nothing durable ever lived there.
